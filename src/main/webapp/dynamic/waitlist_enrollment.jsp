...
<html>
	<head>
		<title>Waitlist Enrollment Form</title>
		
	    <style type="text/css">
   		<%@include file="../css/waitlist_enrollment.css" %></style>
		...
		<tr>
			<form action="waitlist_enrollment.jsp" method="post">
		...
		<%
			...
			String GET_COURSE_QUERY = "select * from section_waitlist";
				...
				<%
					...
					<% 
						if (action != null && action.equals("insert")) {
							...
							PreparedStatement pstmt = conn.prepareStatement("INSERT INTO section_waitlist VALUES (?,?,?,?,?,?,?); ");
							...
							// UPDATE the student attributes in the Student table. 
 							PreparedStatement pstatement = conn.prepareStatement("UPDATE section_waitlist SET units = ?, grade = ? WHERE class_title = ? AND qtr = ? AND year = ? AND section_id = ? AND sid = ?");
							...
							PreparedStatement pstmt = conn.prepareStatement( "DELETE FROM section_waitlist WHERE class_title = ? AND qtr = ? AND year = ? AND section_id = ? AND sid = ?");
